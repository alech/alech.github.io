...
Elm.Date = Elm.Date || {};
Elm.Date.CalendarWeeks = Elm.Date.CalendarWeeks || {};
Elm.Date.CalendarWeeks.make = function (_elm) {
   "use strict";
   _elm.Date = _elm.Date || {};
   _elm.Date.CalendarWeeks = _elm.Date.CalendarWeeks || {};
   if (_elm.Date.CalendarWeeks.values) return _elm.Date.CalendarWeeks.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   ...
   $String = Elm.String.make(_elm);
   var _op = {};
   var weekDays = function (cws) {
      var _p0 = cws;
      switch (_p0.ctor)
      {case "ISO8601": return _U.list([$Date.Mon,$Date.Tue,$Date.Wed,$Date.Thu,$Date.Fri,$Date.Sat,$Date.Sun]);
         case "NorthAmerican": return _U.list([$Date.Sun,$Date.Mon,$Date.Tue,$Date.Wed,$Date.Thu,$Date.Fri,$Date.Sat]);
         default: return _U.list([$Date.Sat,$Date.Sun,$Date.Mon,$Date.Tue,$Date.Wed,$Date.Thu,$Date.Fri]);}
   };
   var firstDayOfWeek = function (cws) {
      var _p1 = cws;
      switch (_p1.ctor)
      {case "ISO8601": return $Date.Mon;
         case "Islamic": return $Date.Sat;
         default: return $Date.Sun;}
   };
   var maybeDate = F3(function (y,m,d) {
      var dayToDateString = F3(function (year,month,day) {
         return A2($Basics._op["++"],
         $Basics.toString(year),
         A2($Basics._op["++"],"/",A2($Basics._op["++"],$Basics.toString(month),A2($Basics._op["++"],"/",$Basics.toString(day)))));
      });
      return $Result.toMaybe($Date.fromString(A3(dayToDateString,y,m,d)));
   });
   var daysInYear = function (year) {
      var daysInMonth = F2(function (y,m) {
         var isLeapYear = function (year) {
            return !_U.eq(A2($Basics._op["%"],year,4),0) ? false : !_U.eq(A2($Basics._op["%"],year,100),0) ? true : !_U.eq(A2($Basics._op["%"],year,400),
            0) ? false : true;
         };
         var daysPerMonth = F2(function (year,month) {
            var _p2 = month;
            switch (_p2)
            {case 1: return 31;
         ...
         return A2($List.filterMap,function (d) {    return A3(maybeDate,y,m,d);},_U.range(1,A2(daysPerMonth,y,m)));
      });
      return $List.concat(A2($List.map,function (month) {    return A2(daysInMonth,year,month);},_U.range(1,12)));
   };
   var cwWeek = function (calendarWeek) {    var _p3 = calendarWeek;return _p3._0.week;};
   var cwYear = function (calendarWeek) {    var _p4 = calendarWeek;return _p4._0.year;};
   var toISO8601WeekString = function (calendarWeek) {
      return A2($Basics._op["++"],
      $Basics.toString(cwYear(calendarWeek)),
      A2($Basics._op["++"],"-W",A2($Basics._op["++"],_U.cmp(cwWeek(calendarWeek),10) < 0 ? "0" : "",$Basics.toString(cwWeek(calendarWeek)))));
   };
   var CalendarWeek = function (a) {    return {ctor: "CalendarWeek",_0: a};};
   var cwFromYearAndWeek = F2(function (year,week) {    return CalendarWeek({year: year,week: week});});
   var daysInYearWithCalendarWeek = F2(function (cws,y) {
      var fixLastNDays = F2(function (cws,days) {
         var lastDayDOW = function () {
            var _p5 = A3(maybeDate,y,12,31);
            if (_p5.ctor === "Nothing") {
                  return $Date.Mon;
               } else {
                  return $Date.dayOfWeek(_p5._0);
               }
         }();
         var daysToFix = function (cws) {
            var _p6 = cws;
            switch (_p6.ctor)
            {case "ISO8601": var _p7 = lastDayDOW;
                 switch (_p7.ctor)
                 {case "Mon": return 1;
                    case "Tue": return 2;
                    case "Wed": return 3;
                    ...
                    case "Sat": return 0;
                    default: return 0;}
               case "NorthAmerican": var _p8 = lastDayDOW;
                 switch (_p8.ctor)
                 {case "Sun": return 1;
                    case "Mon": return 2;
                    case "Tue": return 3;
         ...
         return A2($Basics._op["++"],
         A2($List.take,$List.length(days) - daysToFix(cws),days),
         A2($List.map,
         function (_p10) {
            var _p11 = _p10;
            return {ctor: "_Tuple2",_0: _p11._0,_1: A2(cwFromYearAndWeek,y + 1,1)};
         },
         A2($List.drop,$List.length(days) - daysToFix(cws),days)));
      });
      var isLastWeekCWOfNextYear = F2(function (cws,length) {
         var _p12 = cws;
         if (_p12.ctor === "ISO8601") {
               return _U.cmp(length,3) < 1;
            } else {
               return _U.cmp(length,6) < 1;
            }
      });
      var firstDayOnes = A2($List.indexedMap,
      F2(function (i,b) {    return _U.cmp(i,0) > 0 && b ? 1 : 0;}),
      A2($List.map,function (d) {    return _U.eq($Date.dayOfWeek(d),firstDayOfWeek(cws));},daysInYear(y)));
      var calendarWeeks = function (year) {
         var startWeek = F2(function (cws,y) {
            var _p13 = cws;
            if (_p13.ctor === "ISO8601") {
                  var firstDay = function () {
                     var _p14 = A3(maybeDate,y,1,1);
                     if (_p14.ctor === "Nothing") {
                           return $Date.Mon;
                        } else {
                           return $Date.dayOfWeek(_p14._0);
                        }
                  }();
                  var _p15 = firstDay;
                  switch (_p15.ctor)
         ...
         return A3($List.scanl,F2(function (x,y) {    return x + y;}),A2(startWeek,cws,year),firstDayOnes);
      };
      var lastWeekPrevYear = function (year$) {
         return CalendarWeek({year: year$ - 1,week: A2($Maybe.withDefault,0,$List$Extra.last(calendarWeeks(year$ - 1)))});
      };
      var fixFirstWeek = $List.map(function (w) {    return _U.eq(w,0) ? lastWeekPrevYear(y) : CalendarWeek({year: y,week: w});});
      return A2(fixLastNDays,
      cws,
      A3($List.map2,F2(function (v0,v1) {    return {ctor: "_Tuple2",_0: v0,_1: v1};}),daysInYear(y),A2($List.drop,1,fixFirstWeek(calendarWeeks(y)))));
   });
   var daysInCalendarWeek = F2(function (cws,cw) {
      return A2($List.map,
      $Basics.fst,
      A2($List.filter,
      function (e) {
         return _U.eq($Basics.snd(e),cw);
      },
      A2($Basics._op["++"],
      A2(daysInYearWithCalendarWeek,cws,cwYear(cw) - 1),
      A2($Basics._op["++"],A2(daysInYearWithCalendarWeek,cws,cwYear(cw)),A2(daysInYearWithCalendarWeek,cws,cwYear(cw) + 1)))));
   });
   var calendarWeeksWithDaysInYear = F2(function (cws,year) {
      return A2($List.map,
      function (cw) {
         return {ctor: "_Tuple2",_0: cw,_1: A2(daysInCalendarWeek,cws,cw)};
      },
      A2($List.filterMap,$List.head,$List$Extra.group(A2($List.map,$Basics.snd,A2(daysInYearWithCalendarWeek,cws,year)))));
   });
   var calendarWeeksInYear = F2(function (cws,year) {
      return A2($List.filter,function (e) {    return _U.eq(cwYear($Basics.fst(e)),year);},A2(calendarWeeksWithDaysInYear,cws,year));
   });
   var whichCalendarWeek = F2(function (cws,d) {
      var datesEqual = F2(function (d,e) {
         return _U.eq($Date.year(d),$Date.year(e)) && (_U.eq($Date.month(d),$Date.month(e)) && _U.eq($Date.day(d),$Date.day(e)));
      });
      return A2($Maybe.withDefault,
      CalendarWeek({week: 0,year: 0}),
      $List.head(A2($List.map,
      function (e) {
         return $Basics.snd(e);
      },
      A2($List.filter,function (e) {    return A2(datesEqual,$Basics.fst(e),d);},A2(daysInYearWithCalendarWeek,cws,$Date.year(d))))));
   });
   var fromISO8601WeekString = function (weekString) {
      var weekOffset = function () {    var _p16 = A3($String.slice,4,5,weekString);if (_p16 === "-") {    return 6;} else {    return 5;}}();
      var maybeWeek = $Result.toMaybe($String.toInt(A3($String.slice,weekOffset,weekOffset + 2,weekString)));
      var maybeYear = $Result.toMaybe($String.toInt(A3($String.slice,0,4,weekString)));
      var _p17 = maybeYear;
      if (_p17.ctor === "Nothing") {
            ...
            if (_p18.ctor === "Nothing") {
                  ...
                  return $Maybe.Just(CalendarWeek({year: _p17._0,week: _p18._0}));
               }
         }
   };
   var NorthAmerican = {ctor: "NorthAmerican"};
   var Islamic = {ctor: "Islamic"};
   var ISO8601 = {ctor: "ISO8601"};
   var toISO8601WeekDateString = function (date) {
      var dayOfWeekNr = function (d) {
         var _p19 = d;
         switch (_p19.ctor)
         {case "Mon": return 1;
            case "Tue": return 2;
      ...
      var cw = A2(whichCalendarWeek,ISO8601,date);
      return A2($Basics._op["++"],toISO8601WeekString(cw),A2($Basics._op["++"],"-",$Basics.toString(dayOfWeekNr($Date.dayOfWeek(date)))));
   };
   var fromISO8601WeekDateString = function (weekDateString) {
      var maybeYear = $Result.toMaybe($String.toInt(A3($String.slice,0,4,weekDateString)));
      var dashString = function () {
         var _p20 = $String.length(weekDateString);
         if (_p20 === 8) {
               return A2($Basics._op["++"],
               A3($String.slice,0,4,weekDateString),
               A2($Basics._op["++"],
               "-W",
               A2($Basics._op["++"],A3($String.slice,5,7,weekDateString),A2($Basics._op["++"],"-",A3($String.slice,7,8,weekDateString)))));
            } else {
      ...
      function (d) {
         return _U.eq(toISO8601WeekDateString(d),dashString);
      },
      daysInYear(A2($Maybe.withDefault,0,maybeYear))));
   };
   return _elm.Date.CalendarWeeks.values = {_op: _op
                                           ,cwFromYearAndWeek: cwFromYearAndWeek
                                           ,cwYear: cwYear
                                           ,cwWeek: cwWeek
                                           ,daysInYearWithCalendarWeek: daysInYearWithCalendarWeek
                                           ,calendarWeeksInYear: calendarWeeksInYear
                                           ,calendarWeeksWithDaysInYear: calendarWeeksWithDaysInYear
                                           ,whichCalendarWeek: whichCalendarWeek
                                           ,daysInCalendarWeek: daysInCalendarWeek
                                           ,weekDays: weekDays
                                           ,toISO8601WeekString: toISO8601WeekString
                                           ,toISO8601WeekDateString: toISO8601WeekDateString
                                           ,fromISO8601WeekString: fromISO8601WeekString
                                           ,fromISO8601WeekDateString: fromISO8601WeekDateString
                                           ,ISO8601: ISO8601
                                           ,Islamic: Islamic
...
Elm.Main.make = function (_elm) {
   ...
   $Basics = Elm.Basics.make(_elm),
   $Date = Elm.Date.make(_elm),
   $Date$CalendarWeeks = Elm.Date.CalendarWeeks.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   $Html = Elm.Html.make(_elm),
   ...
   $String = Elm.String.make(_elm);
   var _op = {};
   var footer = A3($Html.node,
   "footer",
   _U.list([$Html$Attributes.$class("footer")]),
   _U.list([A2($Html.p,
   _U.list([]),
   _U.list([$Html.text("Made with ")
           ,A2($Html.a,_U.list([$Html$Attributes.href("http://www.elm-lang.org")]),_U.list([$Html.text("elm")]))
           ,$Html.text(". Uses ")
           ,A2($Html.a,_U.list([$Html$Attributes.href("https://github.com/alech/elm-calendarweeks")]),_U.list([$Html.text("elm-calendarweeks")]))
           ,$Html.text(" by ")
           ,A2($Html.a,_U.list([$Html$Attributes.href("https://www.twitter.com/alech")]),_U.list([$Html.text("@alech")]))
           ,$Html.text(".")]))]));
   var computeCalendarWeek = function (cws) {    return $Maybe.map($Date$CalendarWeeks.whichCalendarWeek(cws));};
   var model = function (dateStr) {
      var maybeDate = $Result.toMaybe($Date.fromString(dateStr));
      var maybeCurrYear = A2($Maybe.map,$Date.year,maybeDate);
      return {date: maybeDate
             ,currentYear: maybeCurrYear
             ,calendarWeek: A2(computeCalendarWeek,$Date$CalendarWeeks.ISO8601,maybeDate)
             ,cwSystem: $Date$CalendarWeeks.ISO8601
             ,cwTable: A2($Maybe.withDefault,
             _U.list([]),
             A2($Maybe.map,$Date$CalendarWeeks.calendarWeeksWithDaysInYear($Date$CalendarWeeks.ISO8601),maybeCurrYear))};
   };
   var update = F2(function (action,model) {
      var _p0 = action;
      switch (_p0.ctor)
      {case "ChangeCWSystem": var _p1 = _p0._0;
           return _U.update(model,
           {cwSystem: _p1
           ,cwTable: A2($Maybe.withDefault,_U.list([]),A2($Maybe.map,$Date$CalendarWeeks.calendarWeeksWithDaysInYear(_p1),model.currentYear))
           ,calendarWeek: A2(computeCalendarWeek,_p1,model.date)});
         case "IncreaseYear": var nextYear = A2($Maybe.map,function (y) {    return y + 1;},model.currentYear);
           return _U.update(model,
           {currentYear: nextYear
           ,cwTable: A2($Maybe.withDefault,_U.list([]),A2($Maybe.map,$Date$CalendarWeeks.calendarWeeksWithDaysInYear(model.cwSystem),nextYear))});
         default: var prevYear = A2($Maybe.map,function (y) {    return y - 1;},model.currentYear);
           return _U.update(model,
           {currentYear: prevYear
           ,cwTable: A2($Maybe.withDefault,_U.list([]),A2($Maybe.map,$Date$CalendarWeeks.calendarWeeksWithDaysInYear(model.cwSystem),prevYear))});}
   });
   var cwToString = function (cw) {
      var str = $Basics.toString($Date$CalendarWeeks.cwWeek(cw));
      return _U.eq($String.length(str),1) ? A2($Basics._op["++"],"0",str) : str;
   };
   var dayToString = function (d) {    var str = $Basics.toString(d);return _U.eq($String.length(str),1) ? A2($Basics._op["++"],"0",str) : str;};
   var monToString = function (d) {
      var _p2 = d;
      switch (_p2.ctor)
      {case "Jan": return "01";
         case "Feb": return "02";
   ...
   var cwEntryToTr = F2(function (_p3,mcw) {
      var _p4 = _p3;
      var _p6 = _p4._0;
      return A2($Html.tr,
      _U.list([$Html$Attributes.$class(function () {
         var _p5 = mcw;
         if (_p5.ctor === "Nothing") {
               return "";
            } else {
               return _U.eq(_p5._0,_p6) ? "info" : "";
            }
      }())]),
      A2($Basics._op["++"],
      _U.list([A2($Html.td,_U.list([]),_U.list([$Html.text(cwToString(_p6))]))]),
      A2($List.map,
      function (d) {
         return A2($Html.td,
         _U.list([]),
         ...
         A2($Basics._op["++"],".",A2($Basics._op["++"],monToString($Date.month(d)),"."))))]));
      },
      _p4._1)));
   });
   var showMaybeCWYearTable = F4(function (cws,my,table,mcw) {
      var _p7 = my;
      if (_p7.ctor === "Nothing") {
            return $Html.text("");
         } else {
            return A2($Html.table,
            _U.list([$Html$Attributes.$class("table table-hover")]),
            _U.list([A2($Html.thead,
                    _U.list([]),
                    _U.list([A2($Html.tr,
                    _U.list([]),
                    A2($Basics._op["++"],
                    _U.list([A2($Html.th,_U.list([]),_U.list([$Html.text("CW")]))]),
                    A2($List.map,
                    function (d) {
                       return A2($Html.th,_U.list([]),_U.list([$Html.text($Basics.toString(d))]));
                    },
                    $Date$CalendarWeeks.weekDays(cws))))]))
                    ,A2($Html.tbody,_U.list([]),A2($List.map,function (e) {    return A2(cwEntryToTr,e,mcw);},table))]));
         }
   });
   var calendarWeekTable = function (model) {
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("row")]),
      _U.list([A4(showMaybeCWYearTable,model.cwSystem,model.currentYear,model.cwTable,model.calendarWeek)]));
   };
   var showMaybeCW = function (mcw) {
      return A2($Maybe.withDefault,
      "",
      A2($Maybe.map,function (cw) {    return A2($Basics._op["++"],"Today\'s calendar week: ",$Basics.toString($Date$CalendarWeeks.cwWeek(cw)));},mcw));
   };
   var dateCWBlurp = F2(function (cws,mDate) {
      var isoDate = function (d) {
         return A2($Basics._op["++"],
         $Basics.toString($Date.year(d)),
         A2($Basics._op["++"],"-",A2($Basics._op["++"],monToString($Date.month(d)),A2($Basics._op["++"],"-",$Basics.toString($Date.day(d))))));
      };
      var _p8 = mDate;
      if (_p8.ctor === "Nothing") {
            return "";
            ...
            if (_p9.ctor === "ISO8601") {
                  ...
                  A2($Basics._op["++"],
                  isoDate(_p10),
                  A2($Basics._op["++"],", also known as ",A2($Basics._op["++"],$Date$CalendarWeeks.toISO8601WeekDateString(_p10),"."))));
               } else {
                  return A2($Basics._op["++"],
                  "Today is ",
                  A2($Basics._op["++"],
                  isoDate(_p10),
                  A2($Basics._op["++"],
                  ", a day in calendar week ",
                  A2($Basics._op["++"],$Basics.toString($Date$CalendarWeeks.cwWeek(A2($Date$CalendarWeeks.whichCalendarWeek,cws,_p10))),"."))));
               }
         }
   });
   var currentDateAndWeekJumbotron = function (model) {
      return A2($Html.div,
      _U.list([$Html$Attributes.$class("jumbotron")]),
      _U.list([A2($Html.h1,_U.list([]),_U.list([$Html.text(showMaybeCW(model.calendarWeek))]))
              ,A2($Html.p,_U.list([]),_U.list([$Html.text(A2(dateCWBlurp,model.cwSystem,model.date))]))]));
   };
   var DecreaseYear = {ctor: "DecreaseYear"};
   var IncreaseYear = {ctor: "IncreaseYear"};
   var ChangeCWSystem = function (a) {    return {ctor: "ChangeCWSystem",_0: a};};
   var navBar = F5(function (address,currYear,prevYear,nextYear,cws) {
      var navBarYears = A2($Html.ul,
      _U.list([$Html$Attributes.$class("nav navbar-nav")]),
      _U.list([A2($Html.li,_U.list([]),_U.list([]))
              ,A2($Html.li,_U.list([]),_U.list([A2($Html.a,_U.list([A2($Html$Events.onClick,address,DecreaseYear)]),_U.list([$Html.text(prevYear)]))]))
              ,A2($Html.li,_U.list([$Html$Attributes.$class("active")]),_U.list([A2($Html.a,_U.list([]),_U.list([$Html.text(currYear)]))]))
              ,A2($Html.li,_U.list([]),_U.list([A2($Html.a,_U.list([A2($Html$Events.onClick,address,IncreaseYear)]),_U.list([$Html.text(nextYear)]))]))]));
      var isActive = F2(function (cws1,cws2) {    return _U.eq(cws1,cws2) ? "active" : "";});
      var navBarCWS = A2($Html.ul,
      _U.list([$Html$Attributes.$class("nav navbar-nav navbar-right")]),
      _U.list([A2($Html.li,
              _U.list([$Html$Attributes.$class(A2(isActive,$Date$CalendarWeeks.ISO8601,cws))]),
              _U.list([A2($Html.a,_U.list([A2($Html$Events.onClick,address,ChangeCWSystem($Date$CalendarWeeks.ISO8601))]),_U.list([$Html.text("ISO-8601")]))]))
              ,A2($Html.li,
              _U.list([$Html$Attributes.$class(A2(isActive,$Date$CalendarWeeks.NorthAmerican,cws))]),
              _U.list([A2($Html.a,
              _U.list([A2($Html$Events.onClick,address,ChangeCWSystem($Date$CalendarWeeks.NorthAmerican))]),
              _U.list([$Html.text("North American")]))]))
              ,A2($Html.li,
              _U.list([$Html$Attributes.$class(A2(isActive,$Date$CalendarWeeks.Islamic,cws))]),
              _U.list([A2($Html.a,
              _U.list([A2($Html$Events.onClick,address,ChangeCWSystem($Date$CalendarWeeks.Islamic))]),
              _U.list([$Html.text("Islamic")]))]))]));
      return A3($Html.node,
      "nav",
      ...
      _U.list([A2($Html.div,
              ...
              _U.list([A2($Html.a,_U.list([$Html$Attributes.$class("navbar-brand")]),_U.list([$Html.text("Calendar weeks")]))]))
              ,A2($Html.div,
              _U.list([$Html$Attributes.id("navbar"),$Html$Attributes.$class("navbar-collapse collapse")]),
              _U.list([navBarYears,navBarCWS]))]))]));
   });
   var view = F2(function (address,model) {
      var nextYear = A2($Maybe.withDefault,"",A2($Maybe.map,function (y) {    return $Basics.toString(y + 1);},model.currentYear));
      var prevYear = A2($Maybe.withDefault,"",A2($Maybe.map,function (y) {    return $Basics.toString(y - 1);},model.currentYear));
      var currYear = A2($Maybe.withDefault,"",A2($Maybe.map,$Basics.toString,model.currentYear));
      return A2($Html.div,
      _U.list([]),
      _U.list([A5(navBar,address,currYear,prevYear,nextYear,model.cwSystem)
              ,A2($Html.div,_U.list([$Html$Attributes.$class("container")]),_U.list([currentDateAndWeekJumbotron(model),calendarWeekTable(model),footer]))]));
   });
   var Model = F5(function (a,b,c,d,e) {    return {date: a,currentYear: b,calendarWeek: c,cwSystem: d,cwTable: e};});
   var currentISODate = Elm.Native.Port.make(_elm).inbound("currentISODate",
   "String",
   function (v) {
      return typeof v === "string" || typeof v === "object" && v instanceof String ? v : _U.badPort("a string",v);
   });
   var main = $StartApp$Simple.start({model: model(currentISODate),view: view,update: update});
   return _elm.Main.values = {_op: _op
                             ,main: main
                             ,Model: Model
                             ,ChangeCWSystem: ChangeCWSystem
                             ,IncreaseYear: IncreaseYear
                             ,DecreaseYear: DecreaseYear
                             ,dateCWBlurp: dateCWBlurp
                             ,showMaybeCW: showMaybeCW
                             ,showMaybeCWYearTable: showMaybeCWYearTable
                             ,monToString: monToString
                             ,dayToString: dayToString
                             ,cwToString: cwToString
                             ,cwEntryToTr: cwEntryToTr
                             ,computeCalendarWeek: computeCalendarWeek
                             ,navBar: navBar
                             ,currentDateAndWeekJumbotron: currentDateAndWeekJumbotron
                             ,calendarWeekTable: calendarWeekTable
                             ,footer: footer
                             ,model: model
                             ,view: view
                             ,update: update};
